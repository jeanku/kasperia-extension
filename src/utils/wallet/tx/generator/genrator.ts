...
class Generator {
  ...
  private tryFinishStandardStageProcessing(
    ...
    console.log("transactionFees    2222:", transactionMass, storageMass, transactionFees, absorbChangeToFees)
    // console.log("finalTransaction:", finalTransaction)

    const PriorityFee = this.finalTransactionPriorityFee?.amount || 0n
    console.log("this.finalTransactionPriorityFee.amount:", PriorityFee)
    console.log("finalTransaction.valueWithPriorityFee:", finalTransaction.valueWithPriorityFee)

    // console.log("PriorityFee > stage.aggregateFees + transactionFees:", stage.aggregateFees + transactionFees)

    const valueWithPriorityFee = PriorityFee > stage.aggregateFees + transactionFees ?
        finalTransaction.valueWithPriorityFee : finalTransaction.valueWithPriorityFee + stage.aggregateFees + transactionFees

    const totalStageValueNeeded = this.finalTransactionPriorityFee?.senderPays()
      ? valueWithPriorityFee : finalTransaction.valueWithPriorityFee;
    ...
    if (reject) {
      ...
      let [transactionFeesNew, changeOutputValue] = (() => {
        switch (this.finalTransactionPriorityFee.source) {
          case FeeSource.SenderPays:
            ...
            console.log("this.finalTransactionPriorityFee.amount", this.finalTransactionPriorityFee.amount)
            const fees = this.finalTransactionPriorityFee.amount > transactionFees ?
                this.finalTransactionPriorityFee.amount : transactionFees + this.finalTransactionPriorityFee.amount;
      ...
      console.log("transactionFeesNew", transactionFeesNew)
      console.log("changeOutputValue", changeOutputValue)
  ...
  private finishRelayStageProcessing(stage: Stage, data: Data): [DataKind, Data] {
    ...
    if (this.context.aggregatedUtxos < 2) {
      ...
      throw new Error(
        `InsufficientFunds222: additional_needed = ${data.transactionFees - data.aggregateInputValue}, origin = "relay"`
      );
    } else {
